...
class TrafficLight:
    ...
    def bulid_LED_number(self,char:str):
        ...
        if char == "0":
            for row in range(50):
                for col in range(40):
                    if row < 4: # 第0行和第1行中的元素都点亮
                        temp_LED.light[row][col] = True
                    if row > 45: # 第18行和第19行中的元素点亮
                        temp_LED.light[row][col] = True
                    if col < 6: # 第0列和第1列中的元素点亮
                        temp_LED.light[row][col] = True
                    if col > 33: # 第13列和第14列中的元素点亮
                        temp_LED.light[row][col] = True
        elif char == "1":
            for row in range(50):
                for col in range(40):
            ...
            for row in range(50):
                for col in range(40):
                    if row < 4:  # 第0行和第1行中的元素都点亮
                        temp_LED.light[row][col] = True
                    if row <23 and col >33:
                        temp_LED.light[row][col] = True
                    if row == 23 or row == 24 or row == 25 or row == 26:
                        temp_LED.light[row][col] = True
                    if row > 26 and col < 6:
                        temp_LED.light[row][col] = True
                    if row > 45:
            ...
            for col01 in range(40):
                if self.number01.light[row][col01] == True:
                    if color == "green":
                        ...
                        print(Fore.GREEN + "袁",end="") # print函数默认换行，是end='\n'在起作用, 定义end=“”可以使打印的字符并列显示
                    elif color == "yellow":
                        # print(Fore.YELLOW + "○", end="")
                        print(Fore.YELLOW + "骏", end="")
                    elif color == "red":
                        # print(Fore.RED + "○", end="")
                        print(Fore.RED + "涛", end="")
                else:
